...
template<class T, class S>
inline void checkbox(std::string&, const char* id, T* val, S T::* f, bool& to_set)
{
    bool temp = (val->*f) > 0;
...
{
    section.add( std::make_unique< advanced_control< T, S > >( name, &slider_int< T, S >, group, field ) );
}
...
inline void add_slider_float( rs2::control_section & section,
                              const char * name, param_group< T > & group, S T::* field )
{
    section.add( std::make_unique< advanced_control< T, S > >( name, &slider_float< T, S >, group, field ) );
}
...
inline void add_checkbox( rs2::control_section & section,
                          const char * name, param_group< T > & group, S T::* field )
{
    section.add( std::make_unique< advanced_control< T, S > >( name, &checkbox< T, S >, group, field ) );
}
...
{
    // Read on the way in, not while building: a collapsed section must not pay for a bulk read
    root.on_open = [&advanced, &amc, &get_curr_advanced_controls]()
    {
