...
namespace rs2
{
                                            ...
                                            std::string & error_message )
    {
        try
        {
            ...
            {
                // Options that cannot be queried (e.g. a MIPI color control with no V4L2 CID mapping) throw
                // here. Isolate per option so one bad control does not drop the rest.
                try
                {
                    auto model = create_option_model( option, opt_base_label, this, s, options_invalidated, error_message );
                    insert_option_model( options_metadata, option->id, std::move( model ) );
                }
