...
void init_frame(py::module &m) {
    ...
    object_detection.def_readwrite("class_id", &rs2_object_detection::class_id)
        ...
        .def_readwrite("image_x", &rs2_object_detection::image_x)
        .def_readwrite("image_y", &rs2_object_detection::image_y)
        .def_readwrite("com_valid", &rs2_object_detection::com_valid)
        .def("__repr__", [](const rs2_object_detection& d) {
                ...
                oss << "class_id: " << d.class_id << ", score: " << d.score << ", top_left: [" << d.top_left_x << ", "
                    ...
                    << "], image_position: [" << d.image_x << ", " << d.image_y << "], com_valid: " << d.com_valid;
            return oss.str();
        });
